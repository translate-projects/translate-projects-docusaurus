...
import {
  ...
  docsTranslate,
  reactTranslate,
  syncResourcesBlogTranslate,
  syncResourcesDocsTranslate,
  syncResourcesFilesJsonTheme,
  syncResourcesReactTranslate,
  translateFilesJsonTheme,
} from '../content';
...
export async function translateProject({
  ...
  if (!defaultLocale) {
    await Logger.error(
      `🛑 required param { config: { defaultLocale: 'es'} } \n`
    );
    return;
  }
  ...
  if (!locales?.length) {
    await Logger.error(
      `🛑 required param { config: { locales: ['en','fr']} } \n`
    );
    return;
  }
  ...
  if (theme_config.enable) {
    ...
    const folderTheme = path.join(
      config_base.outputDir,
      defaultLocale,
